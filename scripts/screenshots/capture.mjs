...
async function main() {
  ...
  // Optional substring filter so you can refresh just one surface after a UI
  // change (e.g. `node scripts/screenshots/capture.mjs terminals`) instead of
  // rewriting every PNG — the contribution heatmap and "time ago" labels are
  // randomized/relative, so a full run churns images that didn't really change.
  const filter = process.argv[2]
  const shots = filter ? SHOTS.filter((s) => s.file.includes(filter)) : SHOTS
