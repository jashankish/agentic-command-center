...
npm run screenshots          # → writes docs/*.png (every surface)
# or: node scripts/screenshots/capture.mjs

# refresh just one surface (substring match on the output filename) — handy
# after a UI change, since a full run also rewrites the randomized heatmap and
# relative-time labels in the other images:
npm run screenshots terminals   # → only docs/screenshot-terminals*.png
```
